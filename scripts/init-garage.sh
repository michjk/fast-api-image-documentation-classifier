...
NODE_ID="$(docker compose exec -T garage garage node id -q | tail -n 1)"

echo "Assigning node ${NODE_ID} to layout..."
docker compose exec -T garage garage layout assign -z "${GARAGE_LAYOUT_ZONE}" -c "${GARAGE_LAYOUT_CAPACITY}" "${NODE_ID}"
docker compose exec -T garage garage layout apply --version 1

echo "Creating API key..."
KEY_INFO="$(docker compose exec -T garage garage key create app-key)"
...
docker compose exec -T garage garage bucket create "${GARAGE_BUCKET}" || true

echo "Granting permissions..."
docker compose exec -T garage garage bucket allow --read --write --owner "${GARAGE_BUCKET}" --key "${ACCESS_KEY_ID}"
